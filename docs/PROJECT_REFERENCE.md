...
**Current status:** Step 17D complete · 308 tests passing · awaiting official data fill (Steps 17E/17F)
...
As of 2026-06-10 (after Step 17D):

### Official Entities

| Dataset | Current count | Required | Status |
|---------|---------------|----------|--------|
| Teams | 48 rows (all blank) | 48 verified | ❌ Placeholder — needs fill |
| Groups | 48 rows (all blank) | 48 (12 × 4) | ❌ Placeholder — needs fill |
| Venues | 12 rows (all TBD) | 16+ verified | ❌ Placeholder — needs fill |
| Fixtures (group stage) | 72 rows (all sample_to_be_verified) | 72 verified | ❌ Placeholder — needs fill |
| Fixtures (knockout) | 0 (not included yet) | 32 placeholders | ❌ Needs fill |
| Players | 65 rows across 17 teams | 1,248 (48 × 26) | ❌ Partial sample — needs fill |
| Player priors | 65 rows (sample) | 1,248 | ❌ Partial sample — needs fill |
...
These groups are sample placeholders. They have **NOT** been verified against FIFA's official draw. Fill `official_fixtures_import_template.csv` with the real draw before running simulations.
...
`official_final_enabled = false` — promotion is blocked until all 15 readiness checks pass.
...
| 17D | `src/official/prepare_population_pack.py`, `src/official/import_templates.py`, `src/official/master_workbook.py`, `src/official/population_guide.py`, `src/official/population_status.py`, `src/official/apply_imports.py`, `src/official/promotion.py`, `data/official/import_templates/*.csv` (6 templates), `scripts/apply_official_import.py`, `scripts/preview_official_import.py`, `scripts/prepare_official_population_pack.py`, `scripts/generate_official_import_templates.py`, `scripts/prefill_known_official_data.py`, `app/pages/14_Official_Data_Population.py`, `data/official/population/FILL_DATA_CHECKLIST.md` |
...
### Step 17E — Fill Teams, Groups, Venues, Fixtures (Data work — no code needed)

**Status:** Blocked on manual data entry from verified FIFA sources.

**What needs to be done:**

1. Open `data/official/import_templates/official_teams_import_template.csv` and fill all 48 rows
2. Fill `official_groups_import_template.csv` (48 rows, 12 × 4 teams — must match teams file)
3. Fill `official_venues_import_template.csv` (16+ stadiums with timezones)
4. Fill `official_fixtures_import_template.csv` (72 group-stage + 32 knockout = 104 total)
5. Apply in order: teams → groups → venues → fixtures
6. Run `python scripts/evaluate_official_final_readiness.py`

**Acceptance criteria:**

| Check | Target |
|-------|--------|
| Teams | 48 rows, zero `sample_to_be_verified` |
| Groups | 12 groups × 4 teams, matching teams file exactly |
| Venues | 16+ with verified timezones (critical for `kickoff_utc`) |
| Fixtures | 72 group-stage + 32 knockout, verified kickoff times |

### Step 17F — Fill Squads and Player Priors (Data work — blocked until 17E passes)
