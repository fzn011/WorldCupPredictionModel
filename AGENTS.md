...
- **Monte Carlo page:** Run/report actions use `st.form` + `form_submit_button` (not bare `st.button` inside tabs). Simulation cap is `MAX_MONTE_CARLO_SIMULATIONS` (5000) in `src/utils/constants.py`. UI default preset is **Quick demo — 10 sims, seed 42** (`MONTE_CARLO_UI_DEFAULT_SIMULATIONS=10`); long runs queue via session state and show a progress bar on the next rerun. Readiness blockers surface at the top via `evaluate_monte_carlo_readiness()`.
